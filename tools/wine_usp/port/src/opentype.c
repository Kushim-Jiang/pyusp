...
/* ChainContextSubst Format 1 (simple / direct-glyph rules) — these are the
 * per-glyph counterparts of the class-based rules above. Wine skipped
 * subtype 1 (FIXME TODO); Mongolian 'rclt' lookups (e.g. 36, 42) rely on it
 * to rewrite the FVS markers so the later lookups can select the final
 * contextual forms. Port addition. */
typedef struct {
    WORD ChainSubRuleCount;
    WORD ChainSubRule[1];
}GSUB_ChainSubRuleSet;
...
typedef struct {
    WORD InputGlyphCount;   /* includes the first (covered) glyph */
    WORD Input[1];          /* remaining input glyphs after the first */
}GSUB_ChainSubRule_2;
...
/* ReverseChainSingleSubst (lookup type 8, format 1) — Port addition. Wine
 * dispatched this type to "Unhandled". Mongolian 'rclt' lookup 40 uses it to
 * rewrite FVS markers (marker.masculine.false) in a single reverse pass.
 * Layout: SubstFormat, Coverage(off), BacktrackGlyphCount, BacktrackCoverage[],
 * LookaheadGlyphCount, LookAheadCoverage[], GlyphCount, Substitute[]. */
typedef struct {
    WORD SubstFormat;
    WORD Coverage;
    WORD BacktrackGlyphCount;
    WORD BacktrackCoverage[1];
}GSUB_ReverseChainSubstFormat1_1;

typedef struct {
    WORD LookaheadGlyphCount;
    WORD LookAheadCoverage[1];
}GSUB_ReverseChainSubstFormat1_2;

typedef struct {
    WORD GlyphCount;
    WORD Substitute[1];
}GSUB_ReverseChainSubstFormat1_3;
 ...
 * GSUB
 **********/
static INT GSUB_apply_lookup(const OT_LookupList* lookup, INT lookup_index, const void *gdef, WORD *glyphs, INT glyph_index, INT write_dir, INT *glyph_count);
...
static INT GSUB_apply_ChainContextSubst(const OT_LookupList* lookup, INT lookup_index, const OT_LookupTable *look, const void *gdef, WORD *glyphs, INT glyph_index, INT write_dir, INT *glyph_count)
{
    ...
    for (j = 0; j < GET_BE_WORD(look->SubTableCount); j++)
    {
        ...
        {
            /* Simple Chaining Context Glyph Substitution (rules store glyph
             * ids directly, no class defs). Port: implemented — previously
             * a TODO/skip. Mongolian 'rclt' lookups 36/42 need it to rewrite
             * the FVS markers that gate the final contextual forms. */
            const GSUB_ChainContextSubstFormat1 *ccf1 = (const GSUB_ChainContextSubstFormat1 *)ccsf1;
            WORD offset, count;
            int cov_index, i;

            TRACE("  subtype 1 (Simple Chaining Context Glyph Substitution)\n");

            offset = GET_BE_WORD(ccf1->Coverage);
            cov_index = GSUB_is_glyph_covered((const BYTE*)ccf1 + offset, glyphs[glyph_index]);
            if (cov_index == -1)
            {
                TRACE("Glyph not covered\n");
                continue;
            }
            if (cov_index >= GET_BE_WORD(ccf1->ChainSubRuleSetCount))
                continue;
            offset = GET_BE_WORD(ccf1->ChainSubRuleSet[cov_index]);
            if (offset == 0)
            {
                TRACE("No rules for glyph\n");
                continue;
            }
            {
                const GSUB_ChainSubRuleSet *csrs = (const GSUB_ChainSubRuleSet *)((const BYTE *)ccf1 + offset);
                count = GET_BE_WORD(csrs->ChainSubRuleCount);
                TRACE("%i rules to check\n", count);

                for (i = 0; i < count; ++i)
                {
                    const GSUB_ChainSubRule_1 *backtrack;
                    const GSUB_ChainSubRule_2 *input;
                    const GSUB_ChainSubRule_3 *lookahead;
                    const GSUB_ChainSubRule_4 *substitute;
                    WORD backtrack_count, input_count, lookahead_count, subst_count;
                    int k;
                    int new_index = GSUB_E_NOGLYPH;

                    offset = GET_BE_WORD(csrs->ChainSubRule[i]);
                    backtrack = (const GSUB_ChainSubRule_1 *)((const BYTE *)csrs + offset);
                    backtrack_count = GET_BE_WORD(backtrack->BacktrackGlyphCount);
                    k = glyph_index + dirBacktrack * backtrack_count;
                    if (k < 0 || k >= *glyph_count)
                        continue;

                    input = (const GSUB_ChainSubRule_2 *)&backtrack->Backtrack[backtrack_count];
                    input_count = GET_BE_WORD(input->InputGlyphCount) - 1;
                    k = glyph_index + write_dir * input_count;
                    if (k < 0 || k >= *glyph_count)
                        continue;

                    lookahead = (const GSUB_ChainSubRule_3 *)&input->Input[input_count];
                    lookahead_count = GET_BE_WORD(lookahead->LookaheadGlyphCount);
                    k = glyph_index + dirLookahead * (input_count + lookahead_count);
                    if (k < 0 || k >= *glyph_count)
                        continue;

                    substitute = (const GSUB_ChainSubRule_4 *)&lookahead->LookAhead[lookahead_count];
                    ...
                    {
                        if (GET_BE_WORD(lookahead->LookAhead[k]) !=
                    ...
                    subst_count = GET_BE_WORD(substitute->SubstCount);
                    /* Port fix: a matched but empty subtable (SubstCount==0)
                     * must not terminate the lookup — keep trying later
                     * rules/subtables that actually substitute. */
                    if (!subst_count)
                        continue;
                    for (k = 0; k < subst_count; ++k)
                    {
        ...
        else if (GET_BE_WORD(ccsf1->SubstFormat) == 2)
        {
            ...
            for (i = 0; i < count; i++)
            {
                ...
                /* Port fix: empty (matched, no-action) rule must not stop the
                 * lookup — try the next rule. */
                if (!substitute_count)
                    continue;
        ...
        else if (GET_BE_WORD(ccsf1->SubstFormat) == 3)
        {
            ...
            DBG("[cc3] L%u subtable j=%d pos=%d gid=%u\n", lookup_index, j, glyph_index, glyphs[glyph_index]);
            ...
            DBG("[cc3]   L%u j=%d back=%u in=%u look=%u gid=%u\n", lookup_index, j, backtrack_count, input_count, lookahead_count, glyphs[glyph_index]);

            for (k = 0; k < backtrack_count; ++k)
            {
                offset = GET_BE_WORD(backtrack->Coverage[k]);
                {
                    /* backtrack glyphs are strictly *before* the input glyph:
                     * start one step back, count non-ignored outward. k=0 is
                     * the nearest non-ignored left neighbour. */
                    int bp = nth_nonignored(gdef, flag, glyphs, *glyph_count, glyph_index + dirBacktrack, dirBacktrack, k);
            ...
            /* Port fix: a matched subtable whose SubstCount==0 is a *guard*:
             * per OT spec the first subtable whose context matches wins, even
             * if it substitutes nothing, so it must stop this lookup for this
             * glyph (NOT fall through to later demoting subtables and NOT
             * return NOGLYPH which used to abort the whole driver). */
            if (!substitution_count)
            {
                DBG("[cc3]   L%u j=%d MATCHED guard (empty) -> stop\n", lookup_index, j);
                return glyph_index;
            }
            for (k = 0; k < substitution_count; ++k)
            {
                unsigned int lookup_index = GET_BE_WORD(substitute->SubstLookupRecord[k].LookupListIndex);
                DBG("[cc3]   L%u j=%d APPLY seq=%u -> lookup %u at gid %u\n", lookup_index, j,
                        GET_BE_WORD(substitute->SubstLookupRecord[k].SequenceIndex), lookup_index, glyphs[glyph_index]);
                unsigned int sequence_index = GET_BE_WORD(substitute->SubstLookupRecord[k].SequenceIndex);
                unsigned int g = glyph_index + write_dir * sequence_index;
...
{
    /* ReverseChainSingleSubst is applied once over the whole run in a single
     * reverse pass (each matched position is substituted with the coverage-
     * parallel Substitute glyph). Re-invocation is idempotent because the
     * output glyphs are not in the input coverage, so it is safe under Wine's
     * per-glyph driver loop. */
    int j;

    (void)glyph_index;
    for (j = 0; j < GET_BE_WORD(look->SubTableCount); j++)
    {
        const GSUB_ReverseChainSubstFormat1_1 *r1;
        WORD back_offs[16], look_offs[16];
        WORD back_cnt, look_cnt, glyph_cnt, cov_off;
        WORD subst_offs;
        const WORD *wp;
        int pos, i;

        r1 = (const GSUB_ReverseChainSubstFormat1_1 *)GSUB_get_subtable(look, j);
        if (GET_BE_WORD(r1->SubstFormat) != 1)
            continue;

        /* Format 1 layout (all big-endian u16, sequential):
         *   substFormat
         *   coverageOffset
         *   backtrackGlyphCount, backtrackCoverageOffsets[]
         *   lookaheadGlyphCount, lookaheadCoverageOffsets[]
         *   glyphCount, substituteGlyphIDs[]
         * NOTE: GET_BE_WORD must receive the *value* (dereferenced), never a
         * pointer, or the pointer's low bits get byte-swapped as garbage. */
        wp = (const WORD *)((const BYTE *)r1 + 2);
        cov_off = GET_BE_WORD(*wp++);
        back_cnt = GET_BE_WORD(*wp++);
        for (i = 0; i < back_cnt && i < 16; i++)
            back_offs[i] = GET_BE_WORD(*wp++);
        look_cnt = GET_BE_WORD(*wp++);
        for (i = 0; i < look_cnt && i < 16; i++)
            look_offs[i] = GET_BE_WORD(*wp++);
        glyph_cnt = GET_BE_WORD(*wp++);
        subst_offs = (WORD)((const BYTE *)wp - (const BYTE *)r1);
        /* substitute array: glyph ids (big-endian), indexed by coverage */
        if (glyph_cnt == 0)
            continue;

        pos = (write_dir > 0) ? *glyph_count - 1 : 0;
        while (pos >= 0 && pos < *glyph_count)
        {
            int ci = GSUB_is_glyph_covered((const BYTE *)r1 + cov_off, glyphs[pos]);
            BOOL ok = TRUE;
            if (ci >= 0 && ci < glyph_cnt)
            {
                for (i = 0; i < back_cnt && ok; i++)
                {
                    int bp = pos - write_dir * (i + 1);
                    if (bp < 0 || bp >= *glyph_count ||
                        GSUB_is_glyph_covered((const BYTE *)r1 + back_offs[i], glyphs[bp]) == -1)
                        ok = FALSE;
                }
                for (i = 0; i < look_cnt && ok; i++)
                {
                    int lp = pos + write_dir * (i + 1);
                    if (lp < 0 || lp >= *glyph_count ||
                        GSUB_is_glyph_covered((const BYTE *)r1 + look_offs[i], glyphs[lp]) == -1)
                        ok = FALSE;
                }
                if (ok)
                    glyphs[pos] = GET_BE_WORD(*((const WORD *)((const BYTE *)r1 + subst_offs + 2 * ci)));
            }
            pos -= write_dir;
        }
    }
    return *glyph_count > 0 ? 0 : GSUB_E_NOGLYPH;
}
...
static INT GSUB_apply_lookup(const OT_LookupList* lookup, INT lookup_index, const void *gdef, WORD *glyphs, INT glyph_index, INT write_dir, INT *glyph_count)
{
    ...
    switch(type)
    {
        ...
        case GSUB_LOOKUP_CONTEXT_CHAINED_REVERSE:

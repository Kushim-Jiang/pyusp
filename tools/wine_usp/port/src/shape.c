...
static OPENTYPE_FEATURE_RECORD mongolian_features[] =
{
    ...
    { MS_MAKE_TAG('c','a','l','t'), 1},
    { MS_MAKE_TAG('r','l','i','g'), 1},
    /* Port: Mongolian 'rclt' (required contextual alternates) selects the
     * final contextual forms after the joined forms. Requires the T6
     * ChainContext format-1 support added to opentype.c (lookups 36/42) and
     * T8 reverse (lookup 40) to rewrite the FVS markers. */
    { MS_MAKE_TAG('r','c','l','t'), 1},
};
...
static const ScriptShapeData ShapingData[] =
{
    ...
    {{ ethiopic_features, 4}, {NULL, 0}, NULL, 0, NULL, NULL},
    {{ mongolian_features, 5}, {NULL, 0}, NULL, 0, ContextualShape_Mongolian, NULL},
    {{ mongolian_features, 5}, {NULL, 0}, NULL, 0, ContextualShape_Mongolian, NULL},
    {{ NULL, 0}, {NULL, 0}, NULL, 0, NULL, NULL},
    {{ NULL, 0}, {NULL, 0}, NULL, 0, NULL, NULL},
    {{ NULL, 0}, {NULL, 0}, NULL, 0, NULL, NULL},
...
static int GSUB_apply_feature_all_lookups(const void *header, const void *gdef, LoadedFeature *feature,
        WORD *glyphs, unsigned int glyph_index, int write_dir, int *glyph_count)
{
    ...
    {
        out_index = OpenType_apply_GSUB_lookup(header, gdef, feature->lookups[i], glyphs, glyph_index, write_dir, glyph_count);
        if (getenv("USP2"))
        {
            int z, show = *glyph_count > 40 ? 40 : *glyph_count;
            fprintf(stderr, "[usp2] @%u all-lookups i=%d/%d:", glyph_index, i + 1, feature->lookup_count);
            for (z = 0; z < show; z++) fprintf(stderr, " %u", glyphs[z]);
            fprintf(stderr, "\n");
        }
    ...
    {
        int out2;
        out2 = GSUB_apply_feature_all_lookups(header, gdef, feature, glyphs, glyph_index, write_dir, glyph_count);
...
static INT apply_GSUB_feature_to_glyph(HDC hdc, SCRIPT_ANALYSIS *psa, ScriptCache* psc, WORD *glyphs, INT index, INT write_dir, INT* glyph_count, const char* feat)
{
    ...
    if (getenv("USP2"))
        fprintf(stderr, "[usp2] apply_feature '%s' at %d -> %s\n", feat, index,
                feature ? "found" : "MISSING");
    if (!feature)
        return GSUB_E_NOFEATURE;

    TRACE("applying feature %s\n",feat);
    return GSUB_apply_feature_all_lookups(psc->GSUB_Table, psc->GDEF_Table, feature, glyphs, index, write_dir, glyph_count);
}
    ...
    if (psc->GSUB_Table)
    {
        ...
        for (lookup_index = 0; lookup_index < feature->lookup_count; lookup_index++)
        {
            ...
            while(i < *pcGlyphs && i >= 0)
            {
                ...
                nextIndex = OpenType_apply_GSUB_lookup(psc->GSUB_Table, psc->GDEF_Table, feature->lookups[lookup_index], pwOutGlyphs, i, write_dir, pcGlyphs);
                if (getenv("USP2"))
                {
                    int z, show = *pcGlyphs > 40 ? 40 : *pcGlyphs;
                    fprintf(stderr, "[usp2] %s#%d/%d:", feat, lookup_index + 1, feature->lookup_count);
                    for (z = 0; z < show; z++) fprintf(stderr, " %u", pwOutGlyphs[z]);
                    fprintf(stderr, "\n");
                }
            ...
            /* port trace: whole-run state after this lookup */
            usp_trace_lookup(feat, lookup_index, feature->lookup_count, pwOutGlyphs, *pcGlyphs);
        }
        return *pcGlyphs;
    }
    return GSUB_E_NOFEATURE;
...
static void Apply_Indic_BasicForm(HDC hdc, ScriptCache *psc, SCRIPT_ANALYSIS *psa, WCHAR* pwChars, INT cChars, IndicSyllable *syllable, WORD *pwOutGlyphs, INT* pcGlyphs, WORD *pwLogClust, lexical_function lexical, IndicSyllable *glyph_index, LoadedFeature *feature )
{
    ...
    while(index <= glyph_index->end)
    {
            INT nextIndex;
            INT prevCount = *pcGlyphs;
            nextIndex = GSUB_apply_feature_all_lookups(psc->GSUB_Table, psc->GDEF_Table, feature, pwOutGlyphs, index, 1, pcGlyphs);
            if (nextIndex > GSUB_E_NOGLYPH)
            {
...
HRESULT SHAPE_ContextualShaping(HDC hdc, ScriptCache *psc, SCRIPT_ANALYSIS *psa, WCHAR* pwcChars, INT cChars, WORD* pwOutGlyphs, INT* pcGlyphs, INT cMaxGlyphs, WORD *pwLogClust)
{
    ...
    if (getenv("USP2"))
        fprintf(stderr, "[usp2] ContextualShaping eScript=%u proc=%p\n", psa ? psa->eScript : 999, ShapingData[psa ? psa->eScript : 0].contextProc);
...
rpRangeProperties = &ShapingData[psa->eScript].defaultTextRange;
    ...
    if (getenv("USP2"))
        fprintf(stderr, "[usp2] ApplyDefaultFeatures eScript=%u cotf=%d\n", psa ? psa->eScript : 999, rpRangeProperties ? rpRangeProperties->cotfRecords : -1);
